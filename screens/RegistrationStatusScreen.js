...
const RegistrationStatus = props => {
  ...
  const [cases , setCases] = useState([]); //jahn list ani hoti wahn array pass hota hy 
  const [caseProgress , setCaseProgress] = useState("");

  useEffect( () => {
    if(CASE && CASE.length){
      const temp = CASE.map( (m , index) => {
        return {
  ...
  const showProgress = (selectedCase) => {
    setCaseId(selectedCase)
      if(selectedCase && INFORMATION && INFORMATION.length){
        const id = selectedCase.split(' - ')[0]
        const temp = INFORMATION.filter(m => m.caseId === id)[0].currentstage
